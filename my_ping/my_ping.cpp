...
{
	// check whether is nullptr
	if (ipstr == nullptr || *ipstr == '\0') 
		return false;
	
	const char* p = ipstr;
	int segmentCount = 0;    // 段数计数器
	int currentValue = 0;    // 当前段的数值
	int digitCount = 0;      // 当前段的数字位数
	...
	while (*p != '\0') {
		if (*p >= '0' && *p <= '9') {
			...
			if (digitCount == 0 || segmentCount >= 3) 
				return false;
			segmentCount++;
			currentValue = 0;
			digitCount = 0;
