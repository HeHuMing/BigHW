...
int readconfig_yzmond_conf(const char* const fname)
{
	...
	/*
	config_file_tools test("test.txt");
	if (test.is_read_succeeded() == 0) {
		cout << "配置文件[test.txt]读取有错." << endl;
		return -1;
	}
	string ret;
	int num = test.item_get_raw("[hhm]", "lar", ret);
	cout << ret << " " << num << endl;
	
	vector<string> ret;
	int num = test.get_all_item("[hhm]", ret);
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	const bool is_help = args[OPT_ARGS_HELP].existed(); //后面多处要用，先取值出来，提高效率
	const bool is_debug = args[OPT_ARGS_DEBUG].existed(); //后面多处要用，先取值出来，提高效率
	const bool is_group_case_sensitive = args[OPT_ARGS_GROUP_NAME_CASE_SENSITIVE].existed(); //后面多处要用，先取值出来，提高效率
	const bool is_item_case_sensitive = args[OPT_ARGS_ITEM_NAME_CASE_SENSITIVE].existed(); //后面多处要用，先取值出来，提高效率
...
test_readcfg --cfgfile httpd.conf --group "" --item AddType --brkchar space --type cstring  //wrong!!!!!!!!!!
pause
