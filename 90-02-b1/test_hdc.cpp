...
int main(int argc, char** argv)
{
	...
	if (test_choise[T_LINE] == '1') {
		...
		for (i = 0; i < 20; i++) {
			...
			y2 = rand() % win_high;
			/* 线的粗细 1~5，颜色随机 */
			hdc_line(x1, y1, x2, y2, 3, RGB(rand() % 256, rand() % 256, rand() % 256));
			Sleep(1);
		}
	...
	if (test_choise[T_MATHCURVE] == '1') {
		...
		struct {
			...
			{2353626,"蝴蝶曲线",hdc_draw_Mathematical_curve_2353626},
			{0,NULL,NULL}
		};
		...
		*/
		...
		//hdc_draw_Mathematical_curve_2353626();
	}
	...
	if (test_choise[T_CUSTOM] == '1') {
		...
		struct {
			...
			{2353626,"写大作业写累的章鱼哥",hdc_draw_cartoon_2353626,400,400},
			{0,NULL,NULL}
		};
		/* 显示显示助教哥哥/姐姐们画的卡通图像
		   注：调用函数前，先要添加对应的cpp并在.h中加函数声明
